...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  let embed = new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setFooter("Endless ❤️ ✩ ΛTRΞUS").setColor(client.randomColor()).setTimestamp();
  ...
  if (db.get(`ayar.isim-yas`)) {
    ...
    yazilacakIsim = `${uye.user.username.includes(ayar.tag) ? ayar.tag : (ayar.ikinciTag ? ayar.ikinciTag : (ayar.tag || ""))} ${isim}`;
      ...
      await uye.roles.add(ayar.erkekRolleri || []).catch();
  await uye.roles.remove(ayar.kizRolleri || []).catch();
  await uye.roles.remove(ayar.teyitsizRolleri || []).catch();
    uye.setNickname(`${yazilacakIsim}`).catch();
    if(ayar.tag && uye.user.username.includes(ayar.tag)) uye.roles.add(ayar.ekipRolu).catch();
  message.channel.send(new MessageEmbed().setColor(client.randomColor()).setTitle("Kayıt Sistemi").setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048})).setFooter("Endless ❤️").setTimestamp().setDescription(`\<a:yildiz:765171982710734848> Kayıt Olan Kullanıcı: ${uye}\n\<a:yildiz:765171982710734848> Güncellenen İsim: \`${yazilacakIsim}\`\n\<a:yildiz:765171982710734848> Verilen Roller: <@&765168946475040768>\n\<a:yildiz:765171982710734848> Kayıt Eden: ${message.author}`)).catch();
    if(ayar.chatKanali && client.channels.cache.has(ayar.chatKanali)) client.channels.cache.get(ayar.chatKanali).send(new MessageEmbed().setColor(client.randomColor()).setDescription(`${uye}**Aramıza <@&765168946475040768> Olarak Katıldı. **`));
};
