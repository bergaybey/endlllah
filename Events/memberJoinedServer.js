...
module.exports = async (member) => {
  ...
  if(jaildekiler.some(x => x.includes(member.id)) || tempjaildekiler.some(x => x.id === member.id)){
    ...
    if(ayarlar.fakeHesapLogKanali && member.guild.channels.cache.has(ayarlar.fakeHesapLogKanali)) return member.guild.channels.cache.get(ayarlar.teyitKanali).send(new MessageEmbed().setAuthor(member.guild.name, member.guild.iconURL({dynamic: true})).setDescription(`${member} üyesi sunucuya katıldı fakat hesabı ${member.client.tarihHesapla(member.user.createdAt)} açıldığı için jaile atıldı!`).setTimestamp().setFooter("Arox ❤️"));
  ...
  .setDescription(`
  **\<a:yildiz:765171982710734848> • Sunucuya hoş geldin ${member}, seninle \`${member.guild.memberCount}\` kişiyiz!**
  **\<a:yildiz:765171982710734848> • Ses kanalına girerek kayıt olabilirsin.**
  **\<a:yildiz:765171982710734848> • Hesabın Açılış Süresi: ${member.client.tarihHesapla(member.user.createdAt)}**
  **\<a:yildiz:765171982710734848> • Hesap ${guvenilirlik ? "Tehlikeli!" : "Güvenli!"}**
  `);
...
member.setNickname(`${member.displayName}`).catch();
